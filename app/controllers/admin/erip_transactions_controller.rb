...
module Admin
  class EripTransactionsController < AdminController
    before_action :set_erip_transaction, only: %i[show edit update destroy]
    ...
    def bepaid_notify
      ...
      if et.status == 'successful'
        ...
        if p.payment_type == 'membership'
          ...
          p.start_date = if last_payment.nil? || (last_payment.end_date < et.paid_at.to_date)
                           et.paid_at.to_date
                         else
                           last_payment.end_date + 1.day
                         end
    ...
    def erip_transaction_params
      params.require(:erip_transaction).permit(:status, :message, :type, :transaction_id, :uid, :order_id, :amount,
                                               :currency, :description, :tracking_id, :transaction_created_at, :expired_at, :paid_at, :test, :payment_method_type, :billing_address, :customer, :payment, :erip)
    end
  end
end
